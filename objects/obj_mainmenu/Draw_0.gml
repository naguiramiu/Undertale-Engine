...
with lan 
{
	var menu_names = [menu_item,menu_stat,menu_cell]
	var ary = array_length(menu_names)
}
...
if !textbox_open && !in_submenu
{
	main_menu_selection = scr_updown_modwrap(main_menu_selection,3,,grayed_menu)
	/// MAIN MENU OPEN SELECTED MENU
	if interact_key 
	{
		if array_contains(grayed_menu,main_menu_selection)
...
var top_menu_num = in_submenu ? 1 : array_length(global.stats.party)

for (var i = 0; i < top_menu_num; i++)
draw_character_stats(i,74 * i)
...
{
	var sw = 1 
	draw_text_color_ext(x + 42,y + 95 + (18 * i),string_upper(menu_names[i]),(array_contains(grayed_menu,i) ? c_gray : c_white),sw)
	var selected = (main_menu_selection == i && !in_submenu) || (main_menu_selection == 1 && i == 1 && in_submenu)
	if (selected)
		draw_sprite(spr_soul_text,0,x + 27,y + 97 + 18 * i)
}
